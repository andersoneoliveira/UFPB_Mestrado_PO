...
#include <cstring>
#include <sstream>
#include <math.h>       /* lround - arredondar: > 5 para baixo, e a partir de 5 para cima*/
...
/*--- função abaixo checa se o valor que está sendo informado pelo usuário é compatível com o tipo de problema escolhido, sinal de restrição informado
	...
	*/
bool valor_ok(int tipo_problema, string sinal_restricao, bool lado_direito, bool misto, double valor, string &msg_erro, bool ha_coef_negativo)
{
	msg_erro = "";
...
int main()
{
	...
	cout << " Esta implementacao tambem funciona com Maximizacao e Minimizacao, na forma padrao (todas as restricoes com sinal de = ) e sinais mistos (=, >= e <=)." << endl;
	cout << endl; cout << endl;
	...
	double vet_MaxZ[6] = { 0 }; /*vetor que irá manter os valores da FO para cálculos. Uso o indice conforme posição do elemento, ou seja,
	...
	double matriz_Restricao_FO[6][8] = { 0, 0 }; /* 6 linhas x 8 colunas. Irei usar a partir do indice=1. Sendo a coluna[6]=conteúdo variavel de folga,
	...
	int n_iteracoes = 0;
	double matriz_Simplex[6][12] = { 0, 0 };
	/*
	...
	*/
	...
	cout.precision(2); //fixo exibição de 2 casas decimais
	...
	if (num_elem_FO > 0)
	{
		...
		/* --- realizo o passo 1
		  ...
		  ---*/
		stringstream xValor; //variável para formatar valores com 2 casas decimais
		if (!e_MinZ)
			v_resultado = " Max Z= ";
		else
		{
			...
			{
				xValor.str("");//limpo a variável do tipo stream
				xValor << fixed << setprecision(2) << vet_MaxZ[indice];
				v_resultado += xValor.str() + "x";
		...
		for (indice = 1; indice <= (num_elem_FO); indice++)
		{
			...
			*/
			...
			{
				xValor << fixed << setprecision(2) << vet_MaxZ[indice];
				v_resultado += "  +  " + xValor.str() + "x" + to_string(indice);
			}
			else
			{
				xValor << fixed << setprecision(2) << vet_MaxZ[indice];
		...
		variável artificial adicionada. Se Maximização: Ma(i) + Ma(i+1), e Se Minimização: -Ma(i) - Ma(i+1)  ...etc*/
		int num_var_artificial = 1;
		if ((tipo_Problema == 1) & (e_misto) & (!e_MinZ)) //Maximização mista
		{
			for (linha = 1; linha <= num_restricoes_FO; linha++)
			{
			...
			if ((tipo_Problema == 1) & (e_misto) & (e_MinZ)) //Minimização mista
			{
				for (linha = 1; linha <= num_restricoes_FO; linha++)
				{
					if ((vet_sinal_Restricao[linha] == "=") || (vet_sinal_Restricao[linha] == ">="))
		...
		while (!fim)  /*--- looping que irá garantir a repetição das etapas de 4 a 8, enquanto houver coeficiente negativo na linha Z ---*/
		{
			...
			/*--- fixo a coluna pivô e calculo a razão para cada linha, exceto a linha 0 = Eq.0 = Z, pelo valor do lado direito(coluna=11), até encontrar
			 o menor resultado > 0. Verifico da linha 1 à 5 ---*/
			int linha_menor_razao = 0;
			double menor_razao = 10000;
			double valor_pivo = 0;
			for (linha = 1; linha <= 5; linha++)
			{
				if ((matriz_Simplex[linha][coluna_maior_negativo] != 0) && ((matriz_Simplex[linha][11] / matriz_Simplex[linha][coluna_maior_negativo]) > 0) 
					&& ((matriz_Simplex[linha][11] / matriz_Simplex[linha][coluna_maior_negativo]) < menor_razao)
					)
				{
			...
			cout << endl;
			double coeficiente_col_pivo = 0.0000;
			for (linha = 0; linha <= 5; linha++)
			{
		...
		*/
		...
		if ((tipo_Problema == 1) & (vet_sinal_Restricao[1] == "=") & (!e_misto))
		{
			cout << "Este programa não calcula os Limites para Max ou Min nao mista com sinal de =." << endl;
		}
		else
		if (tipo_Problema == 1)
		{
			...
			while (!fim)
			{
				...
				else // continuar == 1 - início o cálculo dos limites de variação
				{
					...
					{
						double vet_Delta_b[6] = { 0 }; /*apesar de ser de tamanho 6, irei utilizar apenas 5 posições, sendo a 1a indice=1. facilita acompanhar o contador do For.
													...
													---*/
						double vet_Ranges[6] = { 0 }; /*mesma ideia do int vet_Delta_b[6], porém aqui, para cada looping será armazenado o range de valores
												   para o b(i)*/
